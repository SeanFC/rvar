...
Seasonal_correlated_minimiser::Seasonal_correlated_minimiser(
    ...
    for(auto &it : obs) {
        ...
        if (it->get_alpha() > 1.26) 
            it->set_alpha(1.26);

        if (abs(it->get_sd().get_moisture() - 0.0145) < 0.01) 
            it->get_sd().set_moisture(0.218);
...
void Seasonal_correlated_minimiser::set_scales(double rh_scale, double temp_scale, double grid_scale, int number_of_threads) {
    ...
    Square_f state_distance_grid = get_bessel_correlaton_matrix_f(state_location_grid, grid_scale);
    ...
    printf("LOG: Square rooting matrix B start\n");

    //Square root the B matrix
    M.eigen_square_root_inplace();

    printf("LOG: Square rooting matrix B finished\n");
