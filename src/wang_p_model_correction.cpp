...
float wang_eta_wrt_T_nd(float temp_nd) {
    ...
    return to_find_eta/eta_25 * ( -580.0 / pow( T_C_to_K(25) - viscosity_of_water_ref_num, 2));
}
...
float Moisture_Corrector::true_mi(float annual_precip, const Vec_f &temperature, const Vec_f &sunf, const orbital &orb, const loc &location) {
    Ground_Radiation rad = Ground_Radiation(location, orb);
    
    //Dimensionalise the temperature
    Vec_f temperature_d = Vec_f(temperature);
    for(int i=0; i<temperature_d.get_size(); i++)
        temperature_d.set(i, Wang_Dimension::dim_temp(temperature.get(i)));
    ...
    for(int month_of_year =0; month_of_year<number_of_months_in_year; month_of_year++) {
        if(radiation[month_of_year] >0) { 
            e_s_wrt_T = saturation_vapour_pressure_wrt_temp_nd(temperature.get(month_of_year));
    ...
    float output_mi = annual_precip*I_sc/e_q_sum;
    if(annual_precip < 1)
        output_mi = exp(annual_precip-1)*I_sc/e_q_sum;
    ...
    if(output_mi > 6) {
        ...
        printf("MI %f\n", output_mi);
        printf("P %f\n", annual_precip*I_sc/latent_heat_of_vaporisation_of_water);
...
float Moisture_Corrector::uncorrect_mi(
    ...
    for(int i=0; i<temperature_d.get_size(); i++)
        temperature_d.set(i, temp_scaler*temp_past.get(i));
    ...
    float output_mi = annual_precip_nd*I_sc/e_q_sum;
...
float e_s_wrt_temp_alg(
    ...
    float D_wrt_T      = 0.25 * (xi_wrt_T * (-2)/pow(xi,3) + (0.5*xi*delta_wrt_T*pow(delta, -0.5) - sqrt(delta)*xi_wrt_T)/pow(xi,2) + delta_wrt_T);
...
float Wang_Dimension::dim_precip(float non_P) {
    //output_obs.set_MAP(exp(output_obs.get_MAP())* I_sc/ latent_heat_of_vaporisation_of_water);
    if (non_P < 1)
...
float Wang_Dimension::nondim_precip(float P) {
    if(P < I_sc / latent_heat_of_vaporisation_of_water) {
        if(P != 0) 
